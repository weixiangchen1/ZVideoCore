...
AVCodecContext* ZCodec::CreateCodecContext(int iCodecId, int iCodecType) {
    // 1.寻找编码器
    AVCodec* pCodec = nullptr;
    if (iCodecType == 0) {
        pCodec = avcodec_find_encoder((AVCodecID)iCodecId);
    } else {
        pCodec = avcodec_find_decoder((AVCodecID)iCodecId);
    }
    if (pCodec == nullptr) {
        std::cerr << "avcodec_find_codec failed: " << iCodecId << std::endl;
        return nullptr;
    }
...
void ZCodec::SetCodecContext(AVCodecContext* pCodecCtx) {
...
bool ZCodec::SetAVOpt(const char* pKey, const char* pValue) {
    std::unique_lock<std::mutex> lock(m_mutex);
    if (m_pCodecCtx == nullptr) {
        return false;
    }
...
bool ZCodec::SetAVOpt(const char* pKey, const int iValue) {
    std::unique_lock<std::mutex> lock(m_mutex);
    if (m_pCodecCtx == nullptr) {
        return false;
    }
